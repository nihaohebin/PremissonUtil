    ...
    public static PermissionHelper getInstance(@NonNull Activity context, @NonNull OnPermissionCallback permissionCallback) {
        if (singleton == null) {
            synchronized (PermissionHelper.class) {
                if (singleton == null) {
                    singleton = new PermissionHelper(context,permissionCallback);
                }
            }
        }
        return singleton;
    ...
    public PermissionHelper request(@NonNull Object permissionName) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (permissionName instanceof String) {
                ...
                throw new IllegalArgumentException("Permissions can only be one of these types (String) or (String[])" + ". given type is " + permissionName.getClass().getSimpleName());
            }
        } else {
